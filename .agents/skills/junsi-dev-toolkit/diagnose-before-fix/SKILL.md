...
修复前必须诊断、枚举可能原因、确认方向。修复后必须实际运行验证。
...
呈现原因，问用户倾向哪个。**用户确认后才改。** 问+改在同一回复 = 没问。

### 4. 实施修复

- **改前先 checkpoint**：`git stash` 或 `git commit -m "checkpoint: 改前快照"`
- 最小修改，不重构无关代码
...
详见 `../shared/ai-compliance.md` 中的 Diagnose 部分和通用陷阱。
