...
2. **远程更新预检**：`git fetch` + 查上游近期提交/`CHANGELOG`/issue（可 `web_search`），确认各任务块对应的需求**未被上游最新版实现**；已实现的块直接剔除并提示用户"上游已提供（版本/提交 X），建议升级/改用"，不派发不重复造轮子
3. 输出总体规划：目标、约束、拆分成哪些独立任务块
4. 绘制任务块依赖图（无依赖的块可并行）
...
- [ ] 已输出 `📌 路由宣告: cluster`
- [ ] 已做远程更新预检，确认各任务块上游未实现（已实现的块已剔除并告知用户）
